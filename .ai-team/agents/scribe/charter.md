...
3. **Deduplicate and consolidate decisions.md:**
   ...
   - **Overlapping decisions:** Consolidate them with today's date and credit all authors.
...
5. **Commit `.ai-team/` changes:**
   **IMPORTANT — Windows compatibility:** Do NOT use `git -C {path}`. Do NOT embed newlines in `git commit -m`.
   Instead: `cd` into team root, `git add .ai-team/`, check for staged changes, write commit message to temp file, commit with `-F`.
